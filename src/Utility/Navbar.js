          ...
          {navLinks.map((link, index) => (
            ...
            >
              <button
                ...
                className={`group flex items-center gap-1.5 px-3 xl:px-4 py-2 rounded-lg text-sm xl:text-base font-medium transition-all duration-300 ${
                  isActive(link.path)
                ...
                {link.hasDropdown && (
                  <svg 
                    className={`w-3 h-3 xl:w-4 xl:h-4 transition-transform duration-300 ${
                      (link.dropdownType === 'movies' && showMoviesDropdown) ||
      ...
      <div className="flex items-center gap-1 md:gap-2">
        ...
        <div className="relative" ref={searchRef}>
          <button
            ...
            className="group flex items-center justify-center w-8 h-8 md:w-10 md:h-10 rounded-full hover:bg-white/10 text-white transition-all duration-300 hover:scale-110"
            aria-label="بحث"
          >
              ...
              {(searchQuery.length >= 2 || searchLoading) && (
                <div className="mt-2 bg-black/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl overflow-hidden max-h-96 overflow-y-auto">
                  {searchLoading ? (
                    ...
                    <>
                      <div className="sticky top-0 px-4 py-3 text-xs font-semibold text-gray-400 bg-black/50 backdrop-blur border-b border-white/10">
                      ...
                      {searchResults.map((item) => (
                        <div
                          ...
                          className="flex items-center gap-3 px-4 py-3 hover:bg-white/5 cursor-pointer transition-all duration-200 group"
                        >
                          <img
                            src={
                              item.poster_path
                      ...
                      <div className="sticky bottom-0 bg-black/50 backdrop-blur border-t border-white/10">
                        <button
                          onClick={handleViewAllResults}
                          className="w-full py-3 text-sm text-red-500 hover:text-red-400 hover:bg-white/5 transition-all duration-200 font-medium flex items-center justify-center gap-2"
                        >
                    ...
                    <div className="px-4 py-8 text-center">
                      ...
                      <p className="text-sm text-gray-400 mb-3">لا توجد نتائج لـ "{searchQuery}"</p>
                      <button
                        onClick={handleViewAllResults}
...
>
  ...
  {/* ========== Mobile Menu Overlay ========== */}
  <div
    className={`fixed inset-0 bg-gradient-to-b from-black/80 to-black/40 backdrop-blur-md z-40 lg:hidden transition-all duration-500 ${
  ...
  <div
    dir="rtl"
    className={`fixed top-0 right-0 h-full w-[85%] max-w-sm bg-gradient-to-b from-gray-950 to-black border-l border-white/10 z-50 overflow-y-auto transition-transform duration-500 ease-out shadow-2xl lg:hidden ${
      ...
      {navLinks.map((link, index) => (
        <div key={index} className="border-b border-white/5">
          <button
            ...
            className={`w-full text-right px-5 py-4 text-base font-semibold transition-all duration-200 flex items-center justify-between group ${
              isActive(link.path)
